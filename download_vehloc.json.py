...
def do_it():
    ...
    res = requests.get('https://data.texas.gov/api/views/9e7h-gz56/rows.json?accessType=DOWNLOAD', headers=headers)
    content = res.content[:100] if len(res.content) > 100 else res.content
    print(res.status_code, res.request.url, res.headers, content)

    with open('vehlocjson1/9e7h-gz56-VehLoc-{}.json'.format(arrow.now().timestamp), 'w+') as fh:
